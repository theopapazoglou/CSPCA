"""
Simulation 4 (i.i.d. - correlated features, multivariate response).

DGP:  
X ~ N(0, I_p) for i.i.d., X ~ N(0, Sigma),  Sigma_ij = 0.4^|i-j|  for correlated 
Y1 = exp(X1) +3*sin(X2) - 4*X3 + e1
Y2 = 2*X2 - 0.5*X2 + 4*X4 -  exp(X5) + e2
Y3 =  -X1 + 3*sin(X3) + exp(X6+1) + e3
...
def generate_data(n, p, correlated=False, rho=0.4, noise_sd=0.1,
    ...
    Y = np.column_stack([
            np.exp(X[:, 0]) + 3*np.sin(X[:, 1]) - 4*X[:, 2],                    # Y1
            2*X[:, 1] - 0.5*X[:, 1]**2 + 4*X[:, 3] - np.exp(X[:, 4]),           # Y2
            -X[:, 0] + 3*np.sin(X[:, 2]) + 2*np.exp(X[:, 5] + 1) * 0.5,         # Y3
        ]) + E
    return X, Y
