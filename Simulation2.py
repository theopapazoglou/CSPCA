"""
Simulation 2 (i.i.d. - correlated features, single scalar response).

DGP:  
X ~ N(0, I_p) for i.i.d., X ~ N(0, Sigma),  Sigma_ij = 0.4^|i-j|  for correlated 
Y = 2*X1 - 0.5*X1^2 + 3*X2 + -4*exp(X3+1) + eps,  eps ~ N(0, 0.1^2)
...
def generate_data(n, p, noise_sd=0.1, seed=0):
    ...
    y = 2 * X[:, 0] - 0.5 * X[:, 0]**2 + 3 * X[:, 1] - 4 * np.exp(X[:,2]+1) + eps
